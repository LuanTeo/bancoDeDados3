...
select * from paciente where nome_pac like 'Jo%'and data_nascimento_pac > 1990;
...
select *from paciente where data_nascimento_pac between '1980-01-01' and '1990-12-31' and endereco_pac like '%Rua%';

#6 Selecione os materiais cujo tipo é "Material' e estoque maior que 100.
select * from material where tipo_mat like 'Material' and estoque_mat > 100;

#7 Liste os funcionários com especialidade "Ortodontia" e CPF terminado em "00".
select * from funcionario where especialidade_fun like 'Ortodontia' and cpf_fun like '%00';

#8 Mostre os agendamentos para o tipo "Avaliação" em salas com ID maior que 2.
select * from agenda where tipo_age like 'Avalição' and id_sal_fk >2; 
...
select * from procedimento where tempo_pro > 30 and requisitos_pro like '%Raio-x%';
...
select * from funcionario where data_nascimento_fun between '1980-01-01' and '1995-12-31' and funcao_fun like 'Recepcionista';
...
select * from consulta where forma_pagamento_con like '%cartao%' and valor_con >100;

#20 Liste salas com nome contendo "atendimento" e número de cadeiras maior que 2.
select * from sala where tipo_sal like '%atendimento%' and numero_cadeiras_procedimento_sal > 2;

########## Exercícios de UPDATE

#21 Atualize o valor dos procedimentos com nome contendo "Canal" e tempo maior que 1 hora, aumentando em 10%.
update Procedimento set valor_pro = valor_pro * 1.10 where nome_pro like '%Canal%' and tempo_pro > '01:00:00';

#22 Atualize o status dos recebimentos vencidos (data_vencimento_rec < CURDATE()) e valor maior que 100 para "atrasado".
update recebimento set status_rec = 'atrasado' where data_vencimento_rec < CURDATE() and valor_rec > 100;

#23 Atualize o estoque dos materiais cujo nome contenha "agulha" e tipo seja "Descartável", subtraindo 10 unidades.
update Material set estoque_mat = estoque_mat - 10 where nome_mat like '%agulha%' and tipo_mat = 'Descartável';

#24 Altere o status das caixas com data_fechamento anterior a hoje e saldo final menor que 0 para "fechado com erro".
update Caixa set status_cai = 'fechado com erro' where data_fechamento_cai < CURDATE() and saldo_final_cai < 0;

#25 Atualize o endereço dos pacientes cujo CPF contenha "999" e email contenha "gmail", adicionando " - Atualizado".
update Paciente set endereco_pac = CONCAT(endereco_pac, ' - Atualizado') where cpf_pac like '%999%' and email_pac like '%gmail%';

#26 Atualize o valor das despesas com descrição contendo "água" e valor entre 50 e 100 para 120.
update Despesa set valor_des = 120 where descricao_des like '%água%' and valor_des between 50 and 100;

#27 Mude o nome fantasia dos fornecedores com CNPJ começando com "55" e nome fantasia contendo "Dental" para "Dental Pro".
update Fornecedor set nome_fantasia_for = 'Dental Pro' where cnpj_for like '55%' and nome_fantasia_for like '%Dental%';

#28 Atualize o celular dos funcionários cujo nome contenha "Maria" e função seja "Dentista" para "(00) 00000-0000".
...
#30 Atualize a função dos funcionários com CPF terminado em "11" e que trabalham como "Atendente" para "Atendente Sênior".
...
#32 Mude a forma de pagamento das consultas com valor > 300 e data entre '2024-01-01' e '2024-06-30' para "Pix".
update Consulta set forma_pagamento_con = 'Pix' where valor_con > 300 and data_con between '2024-01-01' and '2024-06-30';

#33 Atualize a quantidade na tabela Consulta_Material para 0 onde a consulta ocorreu em março e material contém "gaze".
update Consulta_Material cm
join Consulta c ON cm.id_con_fk = c.id_con
join Material m ON cm.id_mat_fk = m.id_mat
set cm.quantidade_mat_con = 0 where month(c.data_con) = 3 and m.nome_mat like '%gaze%';

#34 Atualize o status dos pagamentos com data de vencimento no passado e status "pendente" para "vencido".
...
#36 Atualize a especialidade de funcionários cujo nome contenha "Ana" e função seja "Dentista" para "Ortodontista".
update Funcionario set especialidade_fun = 'Ortodontista' where nome_fun like '%Ana%' and funcao_fun = 'Dentista';

#37 Altere a descrição dos procedimentos cujo nome contenha "clareamento" e valor menor que 200.
update Procedimento set descricao_pro = CONCAT(descricao_pro, ' (Promoção)') where nome_pro like '%clareamento%' and valor_pro < 200;

#38 Atualize o número de cadeiras das salas tipo "Padrão" com descrição contendo "pequena" para 1.
update Sala set numero_cadeiras_procedimento_sal = 1 where tipo_sal = 'Padrão' and descricao_sal like '%pequena%';

#39 Corrija o nome dos materiais cujo nome contenha "resnina" e tipo seja "Estético" para "resina".
update Material set nome_mat = replace(nome_mat, 'resnina', 'resina') where nome_mat like '%resnina%' and tipo_mat = 'Estético';

#40 Atualize o valor do procedimento "Cirurgia" com tempo > 1h e requisitos contendo "anestesia" para R$ 500.
update Procedimento set valor_pro = 500 where nome_pro = 'Cirurgia' and tempo_pro > '01:00:00' and requisitos_pro like '%anestesia%';
...
#43 Delete os fornecedores com CNPJ duplicado e nome fantasia contendo "Genérico".
#delete from Fornecedor where cnpj_for in (select cnpj_for from Fornecedor group by cnpj_for having COUNT(*) > 1) and nome_fantasia_for like '%Genérico%';
...
#46 Exclua despesas com data anterior a 2020 e valor menor que 100.
...
#49 Delete registros de Consulta_Procedimento onde o procedimento tem valor < 50 e tempo menor que 15min.
#delete cp from Consulta_Procedimento cp
#join Procedimento p ON cp.id_pro_fk = p.id_pro
#where p.valor_pro < 50 AND p.tempo_pro < '00:15:00';

#50 Delete caixas com status "inativo" e saldo final < 0.
delete from Caixa where status_cai = 'inativo' and saldo_final_cai < 0;

#51 Exclua os pagamentos com status "cancelado" e valor igual a 0.
delete from pagamento where status_pag = 'cancelado' and valor_pag = 0;

#52 Delete os recebimentos com data de vencimento antes de 2020 e status "pago".
delete from recebimento where year(data_vencimento_rec) < 2020 AND status_rec = 'pago';

#53 Exclua registros da tabela Consulta_Material onde o material não existe mais no estoque e quantidade < 1.
##delete cm from Consulta_Material cm left join Material m ON cm.id_mat_fk = m.id_mat where .id_mat IS NULL AND cm.quantidade_mat_con < 1;

#54 Remova compras com valor = 0 e feitas por funcionário com ID nulo.
delete from Compra where valor_com = 0 and id_fun_fk IS NULL;

#55 Delete agendamentos em salas do tipo "Emergência" com data anterior a 2021.
delete a from Agenda a join Sala s ON a.id_sal_fk = s.id_sal where s.tipo_sal = 'Emergência' AND YEAR(a.data_hora_age) < 2021;

#56 Exclua procedimentos com nome contendo "falso" e descrição vazia.
delete from Procedimento where nome_pro like '%falso%' AND (descricao_pro = '' OR descricao_pro IS NULL);

#57 Delete materiais cujo nome contenha "expirado" e estoque < 5.
delete from Material where nome_mat like '%expirado%' AND estoque_mat < 5;

#58 Remova fornecedores com razão social contendo "inativo" e CNPJ inválido (ex: menos de 14 dígitos).
delete from Fornecedor where razao_social_for LIKE '%inativo%'  AND LENGTH(REPLACE(REPLACE(REPLACE(cnpj_for, '.', ''), '/', ''), '-', '')) < 14;

#59 Delete despesas relacionadas a fornecedores que já foram removidos.
delete d from Despesa d left join Fornecedor f ON d.id_for_fk = f.id_for where f.id_for IS NULL;

#60 Exclua pacientes com email inválido (ex: sem "@") e nome contendo "excluir".
